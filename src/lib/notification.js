...
class NotificationWrapper {
  constructor(options) {
    ...
    if (!('Notification' in window)) {
      console.error('[Notification]', 'This browser does not support system notifications.')
    } else {
      this.support = true
      this.granted = Notification.permission === 'granted'
      console.log('[Notification]', 'Permission has set to', this.granted, 'as default.')
    }
  }
  ...
  requestPermission() {
    if (this.support && !this.granted) {
      Notification.requestPermission().then(permission => {
        console.log('[Notification]', 'User has', permission, 'the system notifications.')
  ...
  notify(title, options) {
    if (this.support && this.granted && (!this.backgroundOnly || document.hasFocus())) {
